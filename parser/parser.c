...
{
    int indent;
    ExprNode_Ptr start_ptr, end_ptr, step_ptr, x_ptr, y_ptr, angle_ptr;
...
double *getTmemory()
{
    return &(parser_data.parameter);
}
#endif

void Parser(const char *file_name)
{
    if (InitParser(file_name) == 0)
    {
        return;
    }
    FetchToken();
...
#ifdef _PARSER_WITH_SEMANTICS
    DrawLoop(parser_data.start_ptr, parser_data.end_ptr, parser_data.step_ptr,
...
void destroy_all_tree()
{
    destroy_tree(parser_data.start_ptr);
    destroy_tree(parser_data.end_ptr);
    destroy_tree(parser_data.step_ptr);
    destroy_tree(parser_data.x_ptr);
    destroy_tree(parser_data.y_ptr);
    destroy_tree(parser_data.angle_ptr);
...
void MatchToken(enum Token_Type expected)
{
    ...
    {
        int i;
        for (int i = 0; i < parser_data.indent; i++)
        {
            logPrint(" ");
        }
...
ExprNode_Ptr MakeExprNode(enum Token_Type opcode, ...)
{
    ...
    case FUNC:
        ...
        break;
    default:
        ptr->content.CaseOperator.left =
            (ExprNode_Ptr)va_arg(arg_ptr, ExprNode_Ptr);
        ptr->content.CaseOperator.right =
            (ExprNode_Ptr)va_arg(arg_ptr, ExprNode_Ptr);
        break;
    }
    va_end(arg_ptr);
...
{
    int i;
    for (i = 0; i < parser_data.indent; i++)
    {
        logPrint(" ");
    }
    logPrint("enter in %s\n", x);
...
void tree_trace(ExprNode_Ptr x) { PrintSyntaxTree(x, 0); }
